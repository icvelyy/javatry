...
Здесь представлена таблица с поведением работы программы при разных значениях ```limit```. ```limit``` представляет собой ограничение на количество возвращаемых ресурсов с API запроса. Как видим, ```limit = 35``` является самым эффективным по времени выполнения кода и по количеству произведенных API запросов.
